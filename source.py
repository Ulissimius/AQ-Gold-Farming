...
# Make a one time initialization of focusing visit neihbor text box
...
def farming_loop():
    ...
    while running:
        ...
        # Battle Screen: (IR) buff and overdive -> click attack -> click attack/click done -> click done
        print('Battle Screen')
        while is_gog_alive(): # There will need to be a catch here to handle player death
            ...
            while initialRun: # Activates important buffs/toggles for the loop
                ...
                if shieldBuff and overdrive:
                    initialRun = False
            find_and_press(filePathMain + 'attack_button.png')
        if not is_pc_alive():
...
def is_gog_alive():
    try:
        pyautogui.locateOnScreen('images/1 - main loop images/hp_0.png', confidence=0.95)
    except pyautogui.ImageNotFoundException:
        print('The Shadow Gog lives!')
        return True
    else:
        print('The Shadow Gog is dead!')
        return False
    
def is_pc_alive():
        try:
            pyautogui.locateOnScreen('images/3 - death event images/pc_dead.png', confidence=0.95)
        except pyautogui.ImageNotFoundException:
            print('PC is alive.')
            return True
        else:
